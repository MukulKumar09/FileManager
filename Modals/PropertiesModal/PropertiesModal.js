...
import styles, {backgroundColor, grey} from '../../styles/styles';
  ...
  if (itemsCount == 1) {
    ...
    const getStatsAsync = async () => {
      ...
      await Promise.all(
        ...
        setSize(tempSize);
        if (tempType.every(val => val === tempType[0])) setType(tempType[0]);
        else setType(tempFileCount + ' files, ' + tempFolderCount + ' folders');
      });
    };
    getStatsAsync();
  }
  return (
    <Modal
      ...
      <View
        ...
        <View style={[styles.rowLayout, styles.bigGap, styles.padding]}>
          <MaterialIcon name="details" />
          <Text style={[styles.text, styles.headingText]}>Properties</Text>
        </View>
        <View style={[styles.divider]} />
        <View style={[styles.padding, styles.smallGap, {width: '100%'}]}>
          ...
          <View style={[styles.rowLayout]}>
            ...
            {itemsCount > 1 ? (
              <Text style={[styles.text, styles.wide]}>
                [Multiple items...]
              </Text>
            ) : (
          ...
          {itemsCount > 1 ? null : (
            <>
              <View style={[styles.rowLayout]}>
                <Text
                  style={[styles.text, styles.textDisabled, {width: '40%'}]}>
                  Created Date:{' '}
                </Text>
                <Text style={[styles.text, styles.wide]}>{created}</Text>
              </View>
              <View style={[styles.rowLayout]}>
                <Text
                  style={[styles.text, styles.textDisabled, {width: '40%'}]}>
                ...
                <Text style={[styles.text, styles.wide]}>{modified}</Text>
              </View>
            </>
          )}
        </View>
        <View style={[styles.divider]} />
        <View style={[styles.padding, styles.mediumGap, {width: '100%'}]}>
          <Pressable
            onPress={() => {
              dispatch({
                type: 'PROPERTIESMODAL',
