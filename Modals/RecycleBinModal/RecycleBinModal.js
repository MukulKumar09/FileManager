...
export default function RecycleBinModal() {
  ...
  const handleLongPress = item => {
    if (selectedItems.length > 0) {
      setSelectedItems(
        useRangeSelect(state.recycleBin, [...selectedItems], item),
      );
    } else {
      selectItem(item);
    }
      ...
      <View
        ...
        <View
          ...
          <View style={[styles.rowLayout, styles.bigGap]}>
            ...
            <View>
              <Text style={[styles.text, styles.headingText]}>Recycle Bin</Text>
            ...
            return (
              <Pressable
                ...
                style={[
                  ...
                  styles.wide,
                  selectedItems.some(
                    selectItem => item['path'] === selectItem['path'],
                  ) && styles.listItemHighlight,
                ]}>
                <View style={{width: 30}}>{useIcon(item['fileType'])}</View>
                <Text
                  numberOfLines={2}
                  ellipsizeMode="tail"
        ...
        <View style={[styles.rowLayout, styles.padding, styles.bigGap]}>
          ...
          <Pressable
            onPress={() => {
              ...
              selectedItems.map(
                item =>
                  (tempRecycleBin = tempRecycleBin.filter(
                    rbItem => rbItem['path'] !== item['path'],
                  )),
              );
              dispatch({
                type: 'SETRECYCLEBIN',
